...
def compute_right2left_transformation(R_wi, T_wi, R_wj, T_wj):
    ...
    T_ji = T_wi - R_wi @ np.linalg.inv(R_wj) @ T_wj
    B = np.linalg.norm((np.linalg.inv(R_wi) @ T_wi) - (np.linalg.inv(R_wj) @ T_wj))
    """Student Code Ends"""
...
def compute_rectification_R(T_ji):
    ...
    """Student Code Starts"""
    r_2 = e_i / np.linalg.norm(e_i)
    r_1 = np.cross(r_2, np.array([0, 0, 1]))
    r_1 = r_1 / np.linalg.norm(r_1)
    r_3 = np.cross(r_1, r_2)
    r_3 = r_3 / np.linalg.norm(r_3)

    R_irect = np.array([r_1, r_2, r_3])
    """Student Code Ends"""
...
def sad_kernel(src, dst):
    ...
    for m in range(src.shape[0]):
        for n in range(dst.shape[0]):
            sad[m, n] = np.abs(src[m] - dst[n]).sum()
    """Student Code Ends"""
...
def zncc_kernel(src, dst):
    ...
    for m in range(src.shape[0]):
        for n in range(dst.shape[0]):
            w1 = src[m].mean(axis=0)
            w2 = dst[n].mean(axis=0)
            sig1 = (((src[m] - w1) ** 2).mean(axis=0)) ** 0.5
            sig2 = (((dst[n] - w2) ** 2).mean(axis=0)) ** 0.5
            zncc[m, n] = (((src[m] - w1) * (dst[n] - w2)).sum(axis=0) / (sig1 * sig2 + EPS)).sum()
    """Student Code Ends"""
...
def image2patch(image, k_size):
    ...
    for h in range(image.shape[0]):
        for w in range(image.shape[1]):
            ...
            for k_i in range(-int(k_size/2), int(k_size/2) + 1):
                for k_j in range(-int(k_size/2), int(k_size/2) + 1):
                    if (h + k_i < 0) or (h + k_i >= image.shape[0]) or (w + k_j < 0) or (w + k_j >= image.shape[1]):
                        r.append(0)
                        g.append(0)
                        b.append(0)
                    else:
                        r.append(image[h + k_i, w + k_j, 0])
                        g.append(image[h + k_i, w + k_j, 1])
                        b.append(image[h + k_i, w + k_j, 2])

            r = np.array(r)
            g = np.array(g)
            b = np.array(b)

            patch_buffer[h, w] = np.array([r, g, b]).T
    """Student Code Starts"""
...
def compute_disparity_map(rgb_i, rgb_j, d0, k_size=5, kernel_func=ssd_kernel):
    ...
    lr_consistency_mask = np.zeros((w, h))
    ...
    for u in range(w):
        ...
        disp = np.take_along_axis(disp_candidates, best_matched_right_pixel.reshape(-1, 1), axis=1)
        disp_map[u] = disp.T.squeeze(0)

        best_matched_left_pixel = value.T[best_matched_right_pixel].argmin(axis=1)
        lr_consistency_mask[u] = np.where(best_matched_left_pixel == vi_idx, 1.0, 0.0)

    disp_map = disp_map.T
    lr_consistency_mask = lr_consistency_mask.T
...
def compute_dep_and_pcl(disp_map, B, K):
    ...
    xv, yv = np.meshgrid(np.arange(dep_map.shape[1]), np.arange(dep_map.shape[0]))
    x_map = (xv - K[0, 2]) * dep_map / K[0, 0]
    y_map = (yv - K[1, 2]) * dep_map / K[1, 1]
...
def postprocess(
    ...
    """Student Code Starts"""
    pcl_world = (np.linalg.inv(R_wc) @ (pcl_cam.T - T_wc)).T
    """Student Code Ends"""
